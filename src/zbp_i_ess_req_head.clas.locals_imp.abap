...
* No fine-grained authorization restrictions for Phase 1 - any user
* who can reach this app may create/update/delete/submit/withdraw
* their own requests. Instance-level "requestor only sees their own
* requests" is a separate concern (RAP instance authorization /
* CDS access control on the underlying table), not this global check.
* Revisit if role-based restrictions are needed later.
