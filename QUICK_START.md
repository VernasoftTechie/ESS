...
├─ ZHR_ESS_LOAN_PERSONAL (loan details, 10 fields)
├─ ZHR_ESS_APPR_STEP (approval timeline, 14 fields, 4 indexes)
├─ ZHR_ESS_WF_CONFIG (approval matrix, 14 fields, 3 indexes)
├─ ZHR_ESS_LOAN_PARAM (eligibility, 13 fields, 2 indexes)
├─ ZHR_ESS_INT_RATE (interest rates, 5 fields, 2 indexes)
├─ ZHR_ESS_CUST_FIELD (custom fields, 12 fields, 2 indexes)
├─ ZHR_ESS_LOAN_PERSONAL_CUSTOM (custom values, 4 fields)
├─ ZHR_ESS_VALIDATION_MSG (validation messages, 7 fields, 2 indexes)
└─ ZHR_ESS_CHANGE_LOG (audit trail, 11 fields, 3 indexes)
