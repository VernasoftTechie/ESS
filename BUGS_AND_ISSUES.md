...
## Issue #015 (Preemptive): Service Binding Cannot Be Hand-Authored

- **Date:** 2026-09-03
- **Component:** `ZSD_ESS_REQ_HEAD` (Service Definition, pushed) / Service Binding (deliberately NOT pushed)
- **Status:** Applied proactively — not discovered the hard way this time

### What This Is
Stage 4 needs a Service Definition (exposes the CDS projections as an
OData service) and a Service Binding (publishes it as a live,
Fiori-reachable endpoint). Unlike every prior new object type in this
project, this one came with a **direct, explicit warning already
documented** in the reference repo's own README, from when its author
hit this exact wall:

> The service binding is intentionally not included. ... publishing/
> catalog assignment is environment-specific in a way plain source
> usually isn't. ... Not something to hand-author going forward — it's
> generated by ADT's Publish action, same as the class.

### What Was Done Differently This Time
- **Service Definition** (`zsd_ess_req_head.srvd.srvdsrv` +
  `.srvd.xml`): pushed via file, following the exact confirmed-correct
  shape from the reference repo (which itself needed one correction
  after real activation: `TYPE` is `SRVD/SRV` not a `BDEF`-style guess,
  plus `SRVD_SOURCE_TYPE`/`SRVD_SOURCE_TYPE_DESC` fields, no `LINKS`
  block). Encoding matched exactly too: the `.srvdsrv` source file is
  plain ASCII (no BOM), the `.xml` wrapper has a UTF-8 BOM — verified
  file-by-file against the reference rather than assumed uniform.
- **Service Binding**: **not pushed at all.** Per the reference
  project's own hard-won conclusion, this needs to be created natively
  in ADT (right-click the Service Definition → New Service Binding →
  OData V4 - UI → Publish) — see the response accompanying this commit
  for the exact steps given to the user.

### Why This Matters
This is the first time in the project a "don't push this, create it
natively" lesson was applied **before** burning multiple failed rounds
discovering it ourselves — the exact same category of problem that
cost 5 rounds on the behavior implementation class (Issues #011-#014)
was avoided here by reading the reference repo's documented experience
first, rather than assuming file-push would work for every object type
by default.

### Test Case / Reproduction
1. Pull the repo in ABAPGit, Activate `ZSD_ESS_REQ_HEAD`.
2. Expect it to activate cleanly (same risk profile as CDS/BDEF, and
   the shape is fully verified against a real post-Sync example).
3. Create the Service Binding manually in ADT per the steps given —
   this is expected, not a failure if it doesn't appear via Pull.
